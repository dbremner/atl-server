...
HRESULT CCodeTypeBuilder::Build(CCodeProxy * pCodeProxy, CWSDLDocument *pDoc)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		m_headers.DisableAutoRehash();
		pos = m_headers.GetStartPosition();
		oldpos = NULL;
		while (pos != NULL)
		{
			oldpos = pos;
			CODEHEADERMAP::CPair *p = m_headers.GetNext(pos);
			if (m_pProxy->AddHeader(p->m_value) != NULL)
			{
				m_headers.RemoveAtPos(oldpos);
			}
			else
			{
				hr = E_OUTOFMEMORY;
		...
		if (pMessage != NULL)
		{
			...
			if (pPart != NULL)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					CODEHEADERMAP::CPair *p = m_headers.Lookup(pPart);
					if (p == NULL)
					{
						if (dwCallFlags & CODEFLAG_PID)
						{
...
HRESULT CCodeTypeBuilder::AddHeaderToFunction(CCodeFunction *pCodeFunc, CAutoPtr<CCodeTypedElement> &spElem, CWSDLMessagePart *pPart)
{
	...
	const CODEHEADERMAP::CPair *p = m_headers.Lookup(pPart);
	if (p != NULL)
	{
		pMember = p->m_value;
	}
	...
	if (pMember == NULL)
	{
		if ((FAILED(CreateSafeCppName(spElem->GetSafeName(), spElem->GetName()))) ||
		...
		if (spNewElem != NULL)
		{
			*spNewElem = *spElem;
			...
			if (m_headers.SetAt(pPart, spNewElem) != NULL)
			{
				spNewElem.Detach();
			}
			else
